...
data = jsonRes['data']
data1 = json.loads(data)['areaTree']
data1 = data1[0]['children']
...
for i in range(0,len(data2)):
    if i != (len(data2)-1):
        outstr2 = '\t\t\t\t\t[\''+ str(data2[i]['date']) + '\', '+str(data2[i]['confirm'])+', '+str(data2[i]['suspect'])+', '+str(data2[i]['dead'])+', '+str(data2[i]['heal'])+'],\n'
    else:
        outstr2 = ''
        # 当天的数据通常不准确，暂时不绘制在图中
        #outstr2 = '\t\t\t\t\t[\''+ str(data2[i]['date']) + '\', '+str(data2[i]['confirm'])+', '+str(data2[i]['suspect'])+', '+str(data2[i]['dead'])+', '+str(data2[i]['heal'])+']\n'
    outall2 = outall2+outstr2
...
fid.close() 
# 写入数据集
modifiedStr = oriStr.replace('//dataInsert//',outall2)
# 写入线图和柱图的Y轴最大值和分隔区间
